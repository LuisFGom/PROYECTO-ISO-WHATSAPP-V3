...
  
  // 🔥 CORRECCIÓN CLAVE 1: Estado que guarda el Contacto seleccionado (para ChatWindow)
  const [selectedContact, setSelectedContact] = useState<Contact | null>(null);
  
  // 🔥 CORRECCIÓN CLAVE 2: Estado que guarda el ID del Contacto (para el resaltado en ChatList)
...
  // 🔧 Optimizar escucha de mensajes - solo recargar si es relevante
  useEffect(() => {
...
  // 🔧 Mejorar búsqueda de conversaciones
...
  // 🔥 Handler para seleccionar conversación (desde ChatList)
  const handleConversationClick = (conversation: Conversation) => {
    // 1. Convertir conversación a Contact para ChatWindow
...
    // 🔥 CORRECCIÓN CLAVE 3: Guardar el ID del contacto seleccionado para resaltado
    // ESTA LÍNEA DEBE ASEGURARSE DE EJECUTARSE SIEMPRE PARA CAMBIAR EL RESALTADO.
...
    setSelectedContact(contact);
    // 🔥 CORRECCIÓN CLAVE 4: Guardar el ID del contacto seleccionado
...
    // 🔥 CORRECCIÓN CLAVE 5: Quitar el ID seleccionado al volver
    setSelectedContactId(null); 
    setIsSidebarOpen(true);
  };

  // 🔧 Handler para mensajes enviados - USAR ACTUALIZACIÓN SILENCIOSA
...
  // 🔧 Handler para actualizar contactos en conversaciones
...
  // 🔧 Handler para eliminar contacto de conversaciones
...
  // 🔧 Handler de edición de contacto
...
      }
    } else {
      // Reemplazo de alert por console.error (buena práctica en React/TS)
      console.error(result.error);
    }
  };

  // 🔧 Handler de eliminación de contacto
...
      if (selectedContact?.id === contactId) {
        setSelectedContact(null);
        setSelectedContactId(null); // Quitar resaltado si se elimina
      }
    } else {
      // Reemplazo de alert por console.error (buena práctica en React/TS)
      console.error(result.error);
...
  // Se mantiene para la vista de contactos, pero se puede ignorar en la lógica de renderizado de chats
...
            {/* 🔥 MODIFICACIÓN CLAVE: Cambiamos la lógica de renderizado para chats */}
            {view === 'chats' ? (
              /* En la vista de CHATS, no mostramos el mensaje de "Cargando" nunca,
                simplemente dejamos que ChatList se renderice inmediatamente con [] si no hay datos.
                El hook useConversations se encarga de la carga en segundo plano. */
              <ChatList
...
              />
            ) : isLoading ? (
              /* En la vista de CONTACTOS (que es view !== 'chats'),
                SÍ mantenemos el mensaje de carga si isLoading es true. */
...
              </div>
            ) : (
              // Vista de CONTACTOS (o chats, si ya terminó la carga)
              <ContactList
                contacts={
